...
create table if not exists public.varroa_tests (
  ...
  sample     integer       check (sample is null or sample > 0),  -- erleak (wash) edo egunak (drop)
  notes      text          not null default '',
  username   text          not null,
  created_by uuid          references auth.users(id) on delete set null,
  created_at timestamptz   not null default now()
...
-- ── Varroa tratamenduak / Tratamientos de varroa ───────────────────────────────
...
declare t text;
begin
  foreach t in array array['hives','inspections','expenses','materials','members','velutina_sightings','varroa_tests','varroa_treatments'] loop
...
-- ══════════════════════════════════════════════════════════════════════════════
--  REALTIME — bezeroei aldaketen berri berehala ematen die (30s polling-aren ordez)
--  Realtime: notifica cambios a los clientes al instante (sustituye al polling)
-- ══════════════════════════════════════════════════════════════════════════════
do $$
declare t text;
begin
  ...
  foreach t in array array['hives','inspections','expenses','materials','members',
    ...
    -- replica identity full: DELETE gertaeretan project_id iristen da iragazkirako
